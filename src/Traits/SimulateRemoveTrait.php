...
trait SimulateRemoveTrait {
    public static function selectRegardlessOfFlag(bool $for_update, ...$index): AbstractModel {
        ...
        $result = static::loadFromDB($ob->where, $for_update, static::USE_MASTER_DB_FOR_INDEX_QUERY);
        if (! $result) {
    ...
    public static function select(bool $for_update, ...$index): AbstractModel {
        ...
        if (property_exists($ret->profile, '_simulate_remove_flag') &&
            (int)$ret->profile->_simulate_remove_flag === 1) {
            $exception_name = static::getFactory()->getNotFoundExceptionName();
            throw new $exception_name(...$index);
        }
        return $ret;
    ...
    public function load(bool $for_update = false): AbstractBaseGateway {
        $result = static::loadFromDB($this->where, $for_update, static::USE_MASTER_DB_FOR_INDEX_QUERY);
        if (! $result) {
